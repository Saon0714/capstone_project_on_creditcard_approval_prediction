...
SELECT * FROM
(SELECT * , DENSE_RANK() OVER (ORDER BY Annual_income DESC) DR
FROM data_cleaned ) S
WHERE DR <=5;   -- In this dataset there are many people having same highest salary So DR <= 5 will not give exact 5 records

SELECT * 
FROM data_cleaned
ORDER BY Annual_income DESC
LIMIT 5;         -- This query will give us top 5 People having highest salary
